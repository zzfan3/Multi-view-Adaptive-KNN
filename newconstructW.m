function [W, elapse] = newconstructW(fea,flab,options)
%	Usage:
%	W = constructW(fea,options)
%
%   lab: label of data % new added
...
%                                W_ij = exp(-norm(x_i-x_j)^2/(2t^2)). This
...
if strcmpi(options.NeighborMode,'KNN') && (options.k > 0)
    if strcmpi(options.Metric,'Euclidean')%compare string
        G = zeros(nSmp*(options.k+1),3);
        for i = 1:ceil(nSmp/BlockSize)%integer
            if i == ceil(nSmp/BlockSize)
                smpIdx = (i-1)*BlockSize+1:nSmp;
                dist = EuDist2(fea(smpIdx,:),fea,0);
                dist = full(dist);
%                 [dump, idx] = sort(dist,2); % sort each row and their idx from small to largest
%                 idx = idx(:,1:options.k+1);% top k minimum values
%                 dump = dump(:,1:options.k+1);
                % new added 2022.3
                [dump, idx] = mink(dist',options.k+1);% sort each row, returntop k minimum values and original id
                dump = dump';
                idx = idx';
                %---------
                if ~bBinary
                    ...
                    % new changed 2022.3
                    for id=1:nSmp
                        for it=1:options.k+1
                            if flab(idx(id,it))==flab(id)
                                dump(id,it) = exp(-dump(id,it)/(2*options.t^2))*(1+exp(-dump(id,it)/(2*options.t^2)));
                            else
                                dump(id,it) = exp(-dump(id,it)/(2*options.t^2))*(1-exp(-dump(id,it)/(2*options.t^2)));
                            end
                        end
