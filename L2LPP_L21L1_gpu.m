function [W, funcVal] = L2LPP_L21L1_gpu(X,Y,rho1,rho3,rho4,C,opts)
%% OBJECTIVE
...
%         L{ii} = constructW(X{ii},options); % 修改过，原来代码： L{ii} = constructW(X{ii}',options);
        L{ii} = newconstructW(X{ii},Y{ii},options);
        L{ii}=full(L{ii});
%         C{ii} = L{ii};
...
for t_idx = 1: task_num
    XY{t_idx} = gpuArray(X{t_idx}')*gpuArray(Y{t_idx});
...
% private functions
    ...
    function [grad_W] = gradVal_eval(W)
        if opts.pFlag
            ...
            parfor i = 1:task_num
                grad_W (i, :) = gpuArray(X{i})*(gpuArray(X{i}') * gpuArray*(W(:,i))-gpuArray(Y{i}));
            end
        else
            grad_W = [];
            for i = 1:task_num                
                grad_W = cat(2, grad_W, gpuArray(X{i}')*(gpuArray(X{i}) * gpuArray(W(:,i))-gpuArray(Y{i}))+ rho3*gpuArray(C{i})*gpuArray(W(:,i)) );                
            end
        end
        grad_W = grad_W+ rho_L2 * 2 * W;
        grad_W = gather(grad_W);
    end
...
% smooth part function value.
    function [funcVal] = funVal_eval (W)
        ...
        if opts.pFlag
            parfor i = 1: task_num                
                funcVal = funcVal + 0.5 * norm (gpuArray(Y{i}) - gpuArray(X{i}) * gpuArray(W(:, i)))^2+ 0.5*rho3 * gpuArray(W(:,i)')*gpuArray(C{i})*gpuArray(W(:,i));                
            end
        else
            for i = 1: task_num                
                funcVal = funcVal + 0.5 * norm (gpuArray(Y{i}) - gpuArray(X{i}) * gpuArray(W(:, i)))^2 + 0.5*rho3 * gpuArray(W(:,i)')*gpuArray(C{i})*gpuArray(W(:,i));               
